...
fn exclude_test() {
    ...
    let genres_without_slug_field: Vec<Genre> = api_wrapper
        .genres()
        .fields("name, slug")
        .exclude("slug")
        .limit("3")
    ...
    let expected_result: Vec<Genre> = vec![
        Genre {
            ...
            checksum: None,
            created_at: None,
            name: Some(String::from("Fighting")),
            slug: None,
            updated_at: None,
            url: None,
        },
...
fn where_test() {
    ...
    let test_characters: Vec<Character> = api_wrapper
        .characters()
        .fields("*")
...
fn search_test() {
    ...
    let ryu_character: Vec<Character> = api_wrapper
        .characters()
        .fields("id, name")
        .search("ryu")
        .limit("1")
    ...
    let expected_result: Vec<Character> = vec![Character {
        id: 4975,
        akas: None,
        checksum: None,
        country_name: None,
...
fn sorting_test() {
    ...
    let games_limited_by_5_asc: Vec<Game> = api_wrapper
        .games()
        .fields("name")
        .limit("5")
        .sort_asc("id")
    ...
    let games_desc_order: Vec<Game> = api_wrapper
        .games()
        .fields("name")
        .limit("1")
        .sort_desc("id")
    ...
    let games_offset: Vec<Game> = api_wrapper
        .games()
        .fields("name")
        .limit("5")
        .offset("3")
...
fn json_response_test() {
    ...
    let test_characters: Vec<serde_json::Value> = api_wrapper
        .characters()
...
fn api_error() {
    ...
    let errored_call: Result<Vec<Genre>> = api_wrapper
        .genres()
        .fields("name")
        .search("Puzzle")
        .request();
...
fn automatic_fields_query() {
    ...
    let solid_snake_chars_vec: Vec<Character> = api_wrapper
        .characters()
        .search("Solid Snake")
        .exclude("description")
        .request()
